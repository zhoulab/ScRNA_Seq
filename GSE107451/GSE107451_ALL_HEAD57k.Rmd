...
Objective 1.3 Which increased or decreased (compare with GSE130158)
...
write.xlsx(res1, paste0("./DESeq_result/",File_Title,".xlsx"),rowNames=T)

write.xlsx(res1[res1$'sig'=='up',],paste0("./DESeq_result/",File_Title,"_UP",p_thresh,".xlsx"),rowNames=T)

write.xlsx(res1[res1$'sig'=='down',],paste0("./DESeq_result/",File_Title,"_DOWN",p_thresh,".xlsx"),rowNames=T)
}

```


#Make a function to Run DESeq2 for Pseudo_HEAD_Cells
...
# 
# which(str_split(colnames(Pseudo_HEAD_Cells),pattern = "__",4,simplify =T)[,2]=="DGRP-551")
#                           which(str_split(colnames(Pseudo_HEAD_Cells),pattern = "__",4,simplify =T)[,3]=="female")
#                                              which(str_split(colnames(Pseudo_HEAD_Cells),pattern = "__",4,simplify =T)[,4]=="50")
...
Run_DESeq2_Pseudo_HEAD_Cells <- function(PSEUDO_INPUT,STRAIN,SEX,OLD_AGE,YOUNG_AGE){
  ...
  PSEUDO_INPUT <- PSEUDO_INPUT[,which(str_split(colnames(PSEUDO_INPUT),pattern = "__",4,simplify =T)[,2]==STRAIN)];
  PSEUDO_INPUT <- PSEUDO_INPUT[,which(str_split(colnames(PSEUDO_INPUT),pattern = "__",4,simplify =T)[,3]==SEX)];
  #Obtain the index for old age and young age columns
  index_old <- which(str_split(colnames(PSEUDO_INPUT),pattern = "__",4,simplify =T)[,4]==OLD_AGE);
  index_young <- which(str_split(colnames(PSEUDO_INPUT),pattern = "__",4,simplify =T)[,4]==YOUNG_AGE);
...
#test Run_DESeq2_Pseudo_HEAD_Cells function Using Universally acknowledged thresholds: p<0.05 and log2FC>1 (or you can manually select based on P value in .xlsx that contains both up and down)
```{r}
#This comparison below took 8 minutes
