...
plot_log2bars_Ages <- function(DATA, GENE, STRAIN, SEX, AGE1, AGE2) {
  ...
  DATA <- DATA[,which(str_split(colnames(DATA),pattern = "_",5,simplify =T)[,1]==STRAIN)];
  DATA <- DATA[,which(str_split(colnames(DATA),pattern = "_",5,simplify =T)[,2]==SEX)];
  
  #prepare data for two ages
  DATA_AGE1 <- DATA[,which(str_split(colnames(DATA),pattern = "_",5,simplify =T)[,3]==AGE1)];
  DATA_AGE2 <- DATA[,which(str_split(colnames(DATA),pattern = "_",5,simplify =T)[,3]==AGE2)];
  ...
  barplot(log2(as.matrix(DATA_AGE1[GENE,])),
          ...
          names.arg=str_split(colnames(DATA_AGE1),pattern = "_",5,simplify =T)[,5],
  ...
  barplot(log2(as.matrix(DATA_AGE2[GENE,])),
          ...
          names.arg=str_split(colnames(DATA_AGE2),pattern = "_",5,simplify =T)[,5],
          ylab="log2Expression level", 
          xlab="Clusters",
          col=c("red"),
          cex.names=0.5,
...
#Example
plot_log2bars_Ages (DATA =Pseudo_HEAD_Cells_clusters,
                   STRAIN = "DGRP-551",
                   SEX = "male",
                   GENE ="vir-1",
                   AGE1 ="30", 
