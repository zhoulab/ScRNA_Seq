...
plot_gene_expression_before_after_4ages <- function(df,gene,genotype,sex,first_age,second_age,third_age,fourth_age,TOP_N_CLUSTERS) {
  #print("reminder: first_age should be youngest; forth_age should be oldest")
  library(ggplot2);
  library(dplyr);
  # Filter columns based on the provided inputs (genotype, sex, young and old ages)
  pattern_first <- paste0(genotype, "_", sex, "_", first_age);
  pattern_second <- paste0(genotype, "_", sex, "_", second_age);
  pattern_third <- paste0(genotype, "_", sex, "_", third_age);
  pattern_fourth <- paste0(genotype, "_", sex, "_", fourth_age);
  ...
  # Find the top 7 clusters of a gene's expression in fourth_age (oldest age)
  df_tmp <- df[gene,];
  ...
  #renew variables of fourth_cols and clusters_fourth
  fourth_cols <- grep(pattern_fourth, colnames(df_tmp), value = TRUE);
  clusters_fourth <- sub(paste0("_", fourth_age), "", fourth_cols);
  
  #find common clusters among clusters_first, clusters_second, clusters_third, clusters_fourth
  common_clusters <- intersect(clusters_first, clusters_second);
  common_clusters <- intersect(common_clusters, clusters_third);
  common_clusters <- intersect(common_clusters, clusters_fourth);
  # Subset the data for the common clusters
  ...
  # Mutate plot_data
   plot_data_long <- plot_data %>%
    ...
    mutate(Age = factor(Age, levels = c("First_Age", "Second_Age", "Third_Age", "Fourth_Age"),labels = c(first_age, second_age, third_age, fourth_age)));  # Label ages
  
  # Create the plot using shapes for clusters
  ggplot(plot_data_long, aes(x = Age, y = Expression, group = Cluster, shape = Cluster)) +
    geom_point(size = 3) +
    geom_line() +
    theme_minimal() +
    labs(
      title = paste0("Multi-Age Plot for Gene: ", gene),
      x = "Age Group",
      y = "log2(TPM) Expression Level",
      shape = "Cluster"
    )+
    scale_shape_manual(values=c(33:72,74:123,125:127),
                       breaks = waiver(), 
                       na.value = NA
    );#use ?points and check pch value options
   }
